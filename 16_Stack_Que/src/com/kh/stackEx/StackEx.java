...
Stack(스택) 
	...
	peek()   : 스택이나 큐에서 맨 위 또는 맨 앞에 있는 데이터를 반환하지만 제거하지는 않음
	push() : 스택에서 값을 추가할때는 push 사용하거나 add 를 사용해서 넣음
		주로 스택에서는 push 를 사용
		add 보통 리스트나 컬렉션에서 주로 사용
		Stack 클래스가 Vector 를 상속받아 만들어진 클래스이기 때문에 add 가능
	isEmpty() : 값이 비어있는지 확인
	size() : 크기 확인
...
public class StackEx {
	...
	public static void main(String[] args) {
		...
		System.out.println("가장 마지막에 보여주고 사라진 스택 2의 값 : " + 스택2);
